...
/* 
/in this our header make  our profile api not working remove this currently we implement this later in our code. we need profile but we didnt need right now same in our mobile navigation we need two button "join waitlist" & "become a member"
and rest are same currently dont need of login/register and profile button remove this.
Also in our join waitlist form add a login link or option so that user click on this then create change our join waitlist form into login form in this login form we take users already registred (verified) email id and send verify this mail at the time of login no password need in login. afer verify the mail via otp then our header "join waitlist" button convert int o like profile. in this profile show their details like currently exists with our all new updated logic understand with working api and db. no error come make all proper migrations and migration.ts file.
update our profile and show all new updated data show their role dashboard access link and show all info with their current role give premium rich ui not cheap shadowish, sideborder, or bulky hover shadowish ui  understand
*/