...
import {
  ...
  Eye, X, RefreshCw, FileText, Building2, Users, BookOpen, Briefcase,
} from "lucide-react";
...
interface Child {
  ...
  full_name: string; class_grade: string; admission_number: string;
  school_name: string; school_city: string; apaar_id: string | null;
}
interface ParentReg {
  id: number; first_name: string; last_name: string;
  email: string; phone: string; pan_number: string;
  city: string; state: string; address: string;
  fee_amount: string; fee_period: string;
...
function DSection({ title, color = "text-[#00468e]" }: { title: string; color?: string }) {
  return (
    <p className={`text-[11px] font-extrabold uppercase tracking-widest pt-4 pb-1 first:pt-0 ${color}`}>{title}</p>
  );
}

// ─────────────────────────────────────────────
// Membership steps config
// ─────────────────────────────────────────────

const STEPS = [
  {
    label: "Start Survey", desc: "Share your community needs",
    href: "/survey", Icon: ClipboardList,
    accent: "#d97706", lightBg: "bg-amber-50", border: "border-amber-100",
    iconBg: "bg-amber-100", iconColor: "text-amber-700",
    tag: "Required", tagBg: "bg-amber-100", tagText: "text-amber-700",
    titleColor: "text-amber-900",
  },
  {
    label: "Parent Registration", desc: "Register as a parent member",
    href: "/registration/parent", Icon: UserPlus,
    accent: "#00468e", lightBg: "bg-blue-50", border: "border-blue-100",
    iconBg: "bg-blue-100", iconColor: "text-[#00468e]",
    tag: "For Parents", tagBg: "bg-blue-100", tagText: "text-blue-700",
    titleColor: "text-blue-900",
  },
  {
    label: "Teacher Registration", desc: "Join as an educator",
    href: "/registration/teacher", Icon: GraduationCap,
    accent: "#6d28d9", lightBg: "bg-violet-50", border: "border-violet-100",
    iconBg: "bg-violet-100", iconColor: "text-violet-700",
    tag: "For Teachers", tagBg: "bg-violet-100", tagText: "text-violet-700",
    titleColor: "text-violet-900",
  },
  {
    label: "School Registration", desc: "Enrol your institution",
    href: "/registration/school", Icon: School,
    accent: "#059669", lightBg: "bg-emerald-50", border: "border-emerald-100",
    iconBg: "bg-emerald-100", iconColor: "text-emerald-700",
    tag: "For Schools", tagBg: "bg-emerald-100", tagText: "text-emerald-700",
...
function DetailModal({ modal, onClose }: { modal: ModalData; onClose: () => void }) {
  ...
  const hdrGradient =
    modal.type === "parent"  ? "from-[#00305f] to-[#00468e]" :
    modal.type === "teacher" ? "from-[#3b0764] to-[#6d28d9]" :
                               "from-[#064e3b] to-[#059669]";

  const typeLabel =
    modal.type === "parent" ? "Parent Registration" :
    modal.type === "teacher" ? "Teacher Registration" : "School Registration";

  const mainTitle =
    modal.type === "parent"  ? `${modal.data.first_name} ${modal.data.last_name}` :
    modal.type === "teacher" ? (modal.data as TeacherReg).full_name :
                               (modal.data as SchoolReg).school_name;

  const TypeIcon =
    modal.type === "parent" ? UserPlus :
    modal.type === "teacher" ? GraduationCap : School;
      ...
      <div className="relative bg-white w-full sm:max-w-lg max-h-[94vh] sm:rounded-2xl rounded-t-2xl overflow-hidden flex flex-col z-10 sm:shadow-2xl">
        ...
        <div className={`bg-gradient-to-r ${hdrGradient} px-6 py-5 flex items-center justify-between flex-shrink-0`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white/70 text-xs font-semibold uppercase tracking-widest">{typeLabel}</p>
              <p className="text-white font-bold text-base leading-tight">{mainTitle}</p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 flex items-center gap-2 flex-shrink-0 flex-wrap">
          ...
          <span className="ml-auto text-xs text-slate-400 flex-shrink-0">
            Submitted {fmtDate(modal.data.submitted_at)}
          </span>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          ...
          {modal.type === "parent" && (() => {
            ...
            return <>
              <DSection title="Personal Information" />
              <DRow label="Full Name"  value={`${d.first_name} ${d.last_name}`} />
              <DRow label="Email"      value={d.email} />
              <DRow label="Phone"      value={d.phone} />
              <DRow label="PAN Number" value={d.pan_number} />
              <DRow label="Address"    value={`${d.address}, ${d.city}, ${d.state}`} />
              <DSection title="Support Request" color="text-blue-700" />
              <DRow label="Fee Amount"      value={fmtAmount(d.fee_amount)} />
              <DRow label="Fee Period"      value={d.fee_period} />
              <DRow label="Reason"          value={d.reason_for_support === "other" ? d.other_reason : d.reason_for_support} />
              <DRow label="Repayment"       value={`${d.repayment_duration} months`} />
              <DRow label="Description"     value={d.description} />
              {d.children?.length > 0 && <>
                <DSection title={`Children (${d.children.length})`} color="text-emerald-700" />
                {d.children.map((c, i) => (
                  <div key={i} className="mb-3 rounded-xl bg-slate-50 p-3.5 border border-slate-100">
                    <p className="text-sm font-bold text-slate-700 mb-2">Child {i + 1} — {c.full_name}</p>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                      {[
                        ...
                        ["City", c.school_city],
                        ...(c.apaar_id ? [["Apaar ID", c.apaar_id]] : []),
                      ].map(([k, v]) => (
                        <div key={k}>
                          <span className="text-[11px] text-slate-400 uppercase tracking-wide">{k}</span>
              ...
              <DSection title="Payment" color="text-amber-700" />
              <DRow label="Amount Paid"  value={fmtAmount(d.payment_amount)} />
              <DRow label="Payment Ref." value={<span className="font-mono text-xs">{d.razorpay_payment_id || "—"}</span>} />
              {d.admin_notes && <DRow label="Admin Note" value={<span className="text-amber-700 font-medium">{d.admin_notes}</span>} />}
            </>;
          })()}
          ...
          {modal.type === "teacher" && (() => {
            ...
            return <>
              <DSection title="Personal Information" />
              <DRow label="Full Name"     value={d.full_name} />
              <DRow label="Date of Birth" value={fmtDate(d.dob)} />
              <DRow label="Gender"        value={d.gender} />
              <DRow label="Phone"         value={d.phone} />
              <DRow label="Email"         value={d.email} />
              <DRow label="Address"       value={d.address} />
              <DRow label="State"         value={d.state} />
              <DSection title="Professional Details" color="text-violet-700" />
              <DRow label="Qualification" value={d.qualification === "Other" ? d.other_qualification : d.qualification} />
              <DRow label="Subject"       value={d.subject === "Other" ? d.other_subject : d.subject} />
              <DRow label="Experience"    value={d.experience} />
              <DSection title="Employment" color="text-emerald-700" />
              <DRow label="School"           value={d.school_name} />
              <DRow label="Employee ID"      value={d.employee_id} />
              <DRow label="Employment Type"  value={d.employment_type} />
              <DRow label="Monthly Salary"   value={fmtAmount(d.salary_monthly)} />
              <DRow label="Joining Date"     value={fmtDate(d.joining_date)} />
              <DSection title="Payment" color="text-amber-700" />
              <DRow label="Amount Paid"  value={fmtAmount(d.payment_amount)} />
              <DRow label="Payment Ref." value={<span className="font-mono text-xs">{d.razorpay_payment_id || "—"}</span>} />
              {d.admin_notes && <DRow label="Admin Note" value={<span className="text-amber-700 font-medium">{d.admin_notes}</span>} />}
          ...
          {modal.type === "school" && (() => {
            ...
            return <>
              <DSection title="School Information" color="text-emerald-700" />
              <DRow label="School Name"    value={d.school_name} />
              <DRow label="Type"           value={d.school_type} />
              <DRow label="Established"    value={d.established_year} />
              <DRow label="Board"          value={d.affiliation_board === "Other" ? d.other_affiliation_board : d.affiliation_board} />
              <DRow label="Affiliation ID" value={d.affiliation_id} />
              <DSection title="Contact & Location" color="text-purple-700" />
              <DRow label="Address"   value={`${d.school_address}, ${d.city}, ${d.state} – ${d.pincode}`} />
              <DRow label="Phone"     value={d.contact_number} />
              {d.alternate_contact && <DRow label="Alt. Phone" value={d.alternate_contact} />}
              <DRow label="Email"     value={d.official_email} />
              {d.website_url && <DRow label="Website" value={<a href={d.website_url} target="_blank" rel="noopener noreferrer" className="text-[#00468e] hover:underline">{d.website_url}</a>} />}
              <DSection title="Administration" color="text-[#00468e]" />
              <DRow label="Principal"         value={d.principal_name} />
              <DRow label="Principal Email"   value={d.principal_email} />
              <DRow label="Principal Contact" value={d.principal_contact} />
              <DRow label="Total Students"    value={d.total_students?.toLocaleString("en-IN")} />
              <DRow label="Total Teachers"    value={d.total_teachers?.toLocaleString("en-IN")} />
              {d.infrastructure_details && <DRow label="Infrastructure" value={d.infrastructure_details} />}
              <DSection title="Payment" color="text-amber-700" />
              <DRow label="Amount Paid"  value={fmtAmount(d.payment_amount)} />
              <DRow label="Payment Ref." value={<span className="font-mono text-xs">{d.razorpay_payment_id || "—"}</span>} />
              {d.admin_notes && <DRow label="Admin Note" value={<span className="text-amber-700 font-medium">{d.admin_notes}</span>} />}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex-shrink-0 bg-white">
          <button onClick={onClose} className="w-full py-2.5 bg-[#00305f] hover:bg-[#00468e] text-white rounded-xl text-sm font-semibold transition-colors">
            Close
          </button>
        </div>
      </div>
...
function AppCard({
  iconBg, iconColor, TypeIcon, typeLabel, typeLabelColor,
  title, subtitle, status, paymentStatus, paymentAmount,
  date, onView,
}: {
  iconBg: string; iconColor: string; TypeIcon: any;
  typeLabel: string; typeLabelColor: string;
  title: string; subtitle: string;
  status: string; paymentStatus: string; paymentAmount: string;
  date: string; onView: () => void;
}) {
  return (
    <div className="flex items-start gap-4 py-4 border-b border-slate-100 last:border-0 group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className={`text-xs font-bold ${typeLabelColor}`}>{typeLabel}</span>
        ...
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          ...
          <PayBadge status={paymentStatus} amount={paymentAmount} />
        </div>
      </div>
      ...
      <button
        onClick={onView}
        className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-50 hover:bg-[#00468e] hover:text-white text-slate-600 text-sm font-semibold transition-all flex-shrink-0 mt-1 group-hover:bg-slate-100"
      >
        <Eye size={14} />
...
export default function ProfilePage() {
  ...
  const [activity, setActivity] = useState<{
    parent: ParentReg[]; teacher: TeacherReg[]; school: SchoolReg[];
  } | null>(null);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 text-center max-w-sm w-full">
          ...
          <p className="text-slate-500 text-sm mb-6">Please log in to view your profile and applications.</p>
          <button onClick={() => router.push("/login")} className="w-full bg-[#00468E] hover:bg-[#00305f] text-white py-3 rounded-xl font-semibold text-sm transition-colors">
            Go to Login
          </button>
        </div>
      </div>
  ...
  // Build tabs — only for types that have data; show "All" only when 2+ types
  const typeTabs = [
  ...
  const tabs = typeTabs.length > 1
    ? [{ key: "all" as TabKey, label: "All", count: totalCount }, ...typeTabs]
    : typeTabs;

  // Auto-correct active tab if it no longer applies
    ...
    <>
      ...
      <div className="min-h-screen bg-[#F2F5F9] py-6 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <button
            onClick={() => router.push("/")}
            className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-[#00468E] mb-6 transition-colors"
          >
          ...
          <div className="flex flex-col lg:flex-row gap-5 items-start">
            ...
            <div className="w-full lg:w-64 flex-shrink-0 lg:sticky lg:top-6 flex flex-col gap-4">
              ...
              {/* Profile card */}
              <div className="bg-[#001f4d] rounded-2xl overflow-hidden">
                {/* Avatar + info */}
                <div className="px-5 pt-6 pb-5 flex flex-col items-center text-center">
                  <div className="relative mb-3">
                    ...
                    <span className="absolute bottom-0.5 right-0.5 w-4 h-4 bg-emerald-400 rounded-full border-2 border-[#001f4d]" />
                  </div>
                  <h1 className="text-base font-bold text-white leading-tight">{user.fullName}</h1>
                  <p className="text-xs text-blue-300 mt-1 break-all">{user.email}</p>
                  <div className="mt-3 inline-flex items-center gap-1.5 bg-white/10 border border-white/15 text-white text-xs font-medium px-3 py-1 rounded-full">
              ...
              <div className="bg-white rounded-2xl p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-50">
                    ...
                    <div>
                      <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">Joined</p>
                      <p className="text-sm font-semibold text-slate-700">{joinDate}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-2.5 rounded-xl bg-emerald-50">
                    <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center flex-shrink-0">
                      <Shield size={14} className="text-emerald-600" />
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">Status</p>
                      <p className="text-sm font-semibold text-emerald-700">Active & Verified</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => router.push("/")}
                  className="w-full bg-[#00468e] hover:bg-[#00305f] text-white py-2.5 rounded-xl text-sm font-semibold transition-colors"
                >
                  Back to Home
                </button>
                <button
                  onClick={handleLogout}
                  className="w-full border border-red-200 text-red-500 hover:bg-red-50 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors"
                >
            ...
            <div className="flex-1 flex flex-col gap-4 min-w-0">
              ...
              {/* ── MY APPLICATIONS
                  Shown only when: loading (skeleton) OR has actual data
              ── */}
                ...
                <div className="bg-white rounded-2xl overflow-hidden">
                  ...
                  <div className="bg-[#001f4d] px-6 py-5">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h2 className="text-white font-bold text-lg leading-tight">My Applications</h2>
                          <p className="text-blue-300 text-sm mt-0.5">Live status of all your submissions</p>
                        </div>
                      </div>
                      {activityLoading
                        ? <RefreshCw size={15} className="text-blue-400 animate-spin flex-shrink-0" />
                        : <span className="flex-shrink-0 w-8 h-8 rounded-full bg-white/15 flex items-center justify-center text-sm font-extrabold text-white">{totalCount}</span>
                      }
                    </div>

                    {/* Tabs — only if 2+ type categories */}
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-4 py-1.5 rounded-xl text-sm font-semibold transition-all ${
                              effectiveTab === tab.key
                                ? "bg-white text-[#00305f]"
                                : "bg-white/10 text-blue-200 hover:bg-white/20"
                            }`}
                          >
                            {tab.label}
                            <span className={`text-xs font-extrabold px-1.5 py-0.5 rounded-md ${
                              effectiveTab === tab.key ? "bg-[#00468e] text-white" : "bg-white/15 text-white"
                            }`}>{tab.count}</span>
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="px-6">
                    {activityLoading ? (
                      ...
                      <>
                        {visibleParent.map(reg => (
                          <AppCard
                            key={`p-${reg.id}`}
                            iconBg="bg-blue-50" iconColor="text-[#00468e]" TypeIcon={UserPlus}
                            typeLabel="Parent" typeLabelColor="text-[#00468e]"
                            title={`${reg.first_name} ${reg.last_name}`}
                            subtitle={`${reg.city}, ${reg.state} · ${reg.children?.length || 0} child${(reg.children?.length || 0) !== 1 ? "ren" : ""}`}
                            status={reg.status} paymentStatus={reg.payment_status} paymentAmount={reg.payment_amount}
                            date={reg.submitted_at} onView={() => setModal({ type: "parent", data: reg })}
                          />
                        ))}
                        {visibleTeacher.map(reg => (
                          <AppCard
                            key={`t-${reg.id}`}
                            iconBg="bg-violet-50" iconColor="text-violet-600" TypeIcon={GraduationCap}
                            typeLabel="Teacher" typeLabelColor="text-violet-600"
                            title={reg.full_name}
                            subtitle={`${reg.school_name} · ${reg.subject === "Other" ? reg.other_subject : reg.subject}`}
                            status={reg.status} paymentStatus={reg.payment_status} paymentAmount={reg.payment_amount}
                            date={reg.submitted_at} onView={() => setModal({ type: "teacher", data: reg })}
                          />
                        ))}
                        {visibleSchool.map(reg => (
                          <AppCard
                            key={`s-${reg.id}`}
                            iconBg="bg-emerald-50" iconColor="text-emerald-600" TypeIcon={School}
                            typeLabel="School" typeLabelColor="text-emerald-600"
                            title={reg.school_name}
                            subtitle={`${reg.city}, ${reg.state} · ${reg.total_students?.toLocaleString("en-IN")} students`}
                            status={reg.status} paymentStatus={reg.payment_status} paymentAmount={reg.payment_amount}
                            date={reg.submitted_at} onView={() => setModal({ type: "school", data: reg })}
                          />
                        ))}
                      </>
                    )}
              ...
              <div className="bg-white rounded-2xl overflow-hidden">
                <div className="bg-[#001f4d] px-6 py-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-white font-bold text-lg leading-tight">Become a Member</h2>
                      <p className="text-blue-300 text-sm mt-0.5">Join India's largest school financial inclusion initiative</p>
                    </div>
                  </div>
                </div>
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {STEPS.map(({ label, desc, href, Icon, lightBg, iconBg, iconColor, tag, tagBg, tagText, titleColor }) => (
                    <a
                      key={href}
                      href={href}
                      className={`group flex items-center gap-4 p-4 rounded-xl ${lightBg} hover:opacity-90 transition-opacity cursor-pointer`}
                    >
              ...
              {/* ── INITIATIVE CARD ── */}
              <div className="bg-[#001f4d] rounded-2xl p-6 relative overflow-hidden">
                ...
                <div className="relative mt-4 pt-4 border-t border-white/10">
                  <p className="text-blue-300 text-sm leading-relaxed">
                    ...
                    <span className="text-white font-semibold">SchoolFee.org</span>{" "}
