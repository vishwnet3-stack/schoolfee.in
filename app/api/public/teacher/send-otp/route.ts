// app/api/public/teacher/send-otp/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.execute(
      `UPDATE teacher_email_otps SET used = 1 WHERE email = ? AND used = 0`,
      [normalizedEmail]
    );
    ...
    await db.execute(
      `INSERT INTO teacher_email_otps (email, otp, expires_at) VALUES (?, ?, ?)`,
        ...
        <body style="font-family:'Segoe UI',sans-serif;background:#f4f6f9;margin:0;padding:0;">
          <div style="max-width:500px;margin:30px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.1);">
            <div style="background:linear-gradient(135deg,#00468E,#003570);padding:30px 24px;text-align:center;">
              ...
              <p style="color:rgba(255,255,255,0.8);font-size:13px;margin:6px 0 0;">Schoolfee Teacher Registration</p>
            </div>
